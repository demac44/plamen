...
const LikesList = ({postID, closeList, groupID}) => {
    const {data, loading} = useQuery(groupID ? GET_LIKES_GP : GET_LIKES, {
        variables:{
            postID,
            limit: 20,
            offset:0
    ...
    return (
        <div className='container-users-list flex-col-ctr'>
            <div className='users-list-box'>
                ...
                <div>
                    {((groupID ? data.get_group_post_likes.length : data.get_post_likes.length) === 0) && <p style={styles.emptyList}>No likes</p>}
                    {groupID ? data.get_group_post_likes.map(user => <UserBox user={user} key={user.userID}/>) 
